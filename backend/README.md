...
- **НИУ ВШЭ, все 4 кампуса** (`HSE_MSK`, `HSE_SPB`, `HSE_NN`, `HSE_PERM`) — `pk.hse.ru`,
  JSON-API Angular-приложения; каждый кампус — отдельный вуз с общим классом парсера,
...
Согласие на зачисление — колонка `approval` в строках `get-abit-list`: `+` у
подавших, `Отсутствует` у остальных (раньше сайт писал `Получено`). Поле
`count_agreement` из сводки сайт не заполняет (всегда 0), поэтому число
согласий парсер считает по строкам списка — как в остальных вузах.
...
парсер сверяет `filial` заголовка с городом вуза, а ожидаемый тип списка
выводит из `finance_type` направления («Бюджетная основа» → `Б`, «Контракт» → `К`) —
перепутанный URL (чужой кампус или чужой тип списка) даёт явную ошибку.
...
ВШЭ — единственный вуз проекта с платными списками: они заводятся отдельными
«направлениями» конфига с кодом-суффиксом (`03.03.02-К` «Физика (платное)»,
`finance_type: Контракт`), поэтому в дашборде бюджет и платное смотрятся раздельно.

Покрытие полное: ВСЕ программы бакалавриата всех четырёх кампусов (Москва: 74
программы → 131 направление, СПб: 22 → 43, Нижний Новгород: 16 → 25, Пермь:
10 → 17). Коды направлений ВШЭ не уникальны (под 38.03.01
в Москве 7 программ), поэтому программы одного кода получают суффиксы
(`38.03.01`, `38.03.01-2`, ...). Конфиг генерируется скриптом
[scripts/generate_hse_config.py](scripts/generate_hse_config.py) из каталога API
(`GET /competitve-group` без параметров — весь справочник). Скрипт идемпотентен:
существующие направления узнаются по `external_id` и сохраняют коды (история в БД
не ломается); при появлении новых программ достаточно перезапустить его и
пересобрать образы.
...
Частота парсинга конкретного вуза в проде — необязательное поле
`parse_interval_hours` у вуза (1–24, по умолчанию 24 = раз в сутки в своём
ночном слоте). Его читает генератор таймеров `scripts/install-timers.sh`:
при значении 4 вуз обходится каждые 4 часа от своего слота. После правки
поля таймеры на VPS нужно перегенерировать (`sudo bash scripts/install-timers.sh`).
...
| HSE_MSK / HSE_SPB | `external_id` = `setId/groupId` | Открыть список направления на `pk.hse.ru`, взять оба UUID из URL `/applicants/<setId>/<groupId>`; кампус и тип списка парсер сверит сам. Платный список = отдельное направление: код с суффиксом `-К`, `finance_type: Контракт` |
